...
void add_reg (FILE *arq, registro reg_s, int index_l) {
    ...
    /* Verifica disponibilidade da posição. Se resposta da leitura do arquivo
    na posição apontada for igual a 1, então a posição já foi explorada */
    if((fread(&reg_aux, sizeof(registro), 1, arq)) == 1) {
        /* Se 'status' do registro for igual a 0, então a posição já foi 
        utilizada, mas no momento está livre */
        if (reg_aux.status == 0) {
            fseek(arq, sizeof(registro)*index_l, SEEK_SET);
            reg_s.pos = index_l;
            reg_s.status = 1;
...
void consult_reg(FILE *arq, int index_c) {
    ...
    /* Posiciona ponteiro na posição requerida e faz
    a leitura */
    ...
    /* Se diferente de 0, posição já utilizada. Verifica se atualmente está
    livre ou ocupada */   
    else {
        fseek(arq, sizeof(registro)*index_c, SEEK_SET);
...
// Função para remo~ção de registro do arquivo 
void remove_reg(FILE *arq, int index_r) {
    ...
    /* Ler o arquivo na posição requerida, se a resposta for diferente de 1
    então a posição não foi descoberta ainda */
    if((fread(&reg_rem, sizeof(registro), 1, arq)) != 1)
        printf("%s %d\n", "posicao ja estava vazia:", index_r);

    /* Se igual a 1, posição já explorada. Verifica se atualmente está
    livre ou ocupada */ 
    else {
        fseek(arq, sizeof(registro)*index_r, SEEK_SET);
